...
{
    vector<string> file;
    ifstream ifile("/Users/wanglinhao/Desktop/retailx.txt");
    if (!ifile)
    {
...
vector<vector<int> > Get_FPfile(vector<string> &file, vector<listNode> &L1)
{ // 第二次扫描数据库 删除非频繁一项
    string temp;
...
vector<string> GetFrequentItems(listNode &lk, FPtreeNode *&head)
{ // 生成条件模式基 rfile
